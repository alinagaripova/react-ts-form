...
function SalaryCalculation({ salaryCalculation, salaryValue = '', isToogled }: SalaryCalculationProps) {
  let salaryValueNumber: number = salaryValue ? parseInt(salaryValue.replace(/\s/g, "")) : 0; // remove separated sign and make number
  let salary: number = 0;
  let tax: number = 0; 
  ...
  return (
    <div className="salary-data">
      <div className="salary-data__el">
        <span>{separateNumber(salary.toString())} ₽</span> сотрудник будет получать на руки
      </div>
      <div className="salary-data__el">
        <span>{separateNumber(tax.toString())} ₽</span> НДФЛ, {salaryCalculation?.percent}% от оклада
      </div>
      <div className="salary-data__el">
        <span>{separateNumber(finishSalary.toString())} ₽</span> за сотрудника в месяц
      </div>
    </div>
  );
}
